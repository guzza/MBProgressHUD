...
- (void)drawBlurredBackgroundInRect:(CGRect)rect
{
    ...
    UIImage *blur = [self screenshot:viewToRender];
    UIImage *outputImage = [self boxblurImageWithBlur:0.2f forImage:blur];
    self.hidden = NO;

    [outputImage drawInRect:rect];
}
-(UIImage *)boxblurImageWithBlur:(CGFloat)blur forImage:(UIImage *)dimage {
...
- (UIImage*)screenshot:(UIView *)dview {
    UIGraphicsBeginImageContext(dview.bounds.size);
    if( [dview respondsToSelector:@selector(drawViewHierarchyInRect:afterScreenUpdates:)] ){
        [dview drawViewHierarchyInRect:dview.bounds afterScreenUpdates:YES];
    }else{
